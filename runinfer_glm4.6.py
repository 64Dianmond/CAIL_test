...
更新日志 (适配 glm-4.6):
- 移除了 requests 库, 替换为 openai 库
- API 配置更新为 DashScope (glm-4.6)
- _call_new_api_backend 函数重写, 使用 client.chat.completions.create
- 明确设置 stream=False, 并添加 extra_body={"enable_thinking": False}
"""
...
# 更新输出文件名以反映版本
OUTPUT_PATH = "result/submission_glm4.6_driven_v3_expert_rules.jsonl"  # 文件名更新

# --- DashScope API 配置 (glm-4.6) ---
DASHSCOPE_API_KEY = "sk-cfeee03df3e14e238a528b50d0d978e9" # 优先使用 DASHSCOPE_API_KEY
DASHSCOPE_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
DASHSCOPE_MODEL_NAME = "glm-4.6"
# NEWAPI_CHAT_COMPLETIONS_ENDPOINT = "/v1/chat/completions" # openai库不需要这个

TEMPERATURE = 0.1
MAX_TOKENS = 8192
MAX_TOKENS_TASK2 = 8192  # T2的Prompt现在更长, 但仍在token限制内
...
class LLMDrivenInferencer:
    ...
    def __init__(self, api_key: str = DASHSCOPE_API_KEY, base_url: str = DASHSCOPE_BASE_URL,
        ...
        print(f"初始化 DashScope 客户端 (v3 - 专家规则版 - {model})")
    ...
    Optional[dict]:
        """封装对DashScope API的HTTP POST请求 (使用openai库)"""
        try:
            ...
            # 将 Pydantic 模型转换为 dict，以便与脚本的其余部分兼容
    ...
    def predict_sentence(self, fact: str, labels: List[str], max_retries: int = 3) -> Tuple[int, int]:
        ...
        # 3. API 调用与解析 (
        # 3. API 调用与解析 (与之前相同, 因为 _call_new_api_backend 返回兼容的 dict)
        for attempt in range(max_retries):
            try:
                response_json = self._call_new_api_backend(
                    messages=messages,
...
def main():
    print("\n" + "=" * 60)
    print("刑事案件量刑辅助系统 - 大模型驱动版本 (v3 - 专家规则 - glm-4.6)")
    print("=" * 60 + "\n")

    # 初始化LLMDrivenInferencer时，它会使用全局定义的 DASHSCOPE_XXX 变量
